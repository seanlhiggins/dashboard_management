...
export function sdkError(response) {
  ...
  if ('error' in response) {
    ...
    if ('statusMessage' in _error) {
      return new Error(_error.statusMessage);
    }

    if ('error' in _error && _error.error instanceof Buffer) {
      var result = Buffer.from(_error.error).toString('utf-8');
      return new Error(result);
    }

    if (_error instanceof Buffer) {
      var _result = Buffer.from(_error).toString('utf-8');

      return new Error(_result);
    }
  ...
  return new Error("Unknown error with SDK method ".concat(error));
}
